...
LDSCRIPTS_DIR = platform.get_package_dir("tool-ldscripts-artery32")
...
sources_path = os.path.join(CMSIS_DEVICE_DIR, "Source") #YSZ-WORK:del Templates
...
env.BuildSources(
    ...
    src_filter=[
        "-<*>",
        "+<%s>" % board.get("build.cmsis.system_file", "system_%s.c" % product_line.lower()), #YSZ-WORK:"system_%sxx.c" % mcu[0:7]->"system_%s.c" % product_line.lower()
        "+<gcc/%s>"
        % board.get(
